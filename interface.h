...
void read_sb(struct struct_ext2_filsys file_sys)
{
    ...
    if (extXdetector(*(file_sys.super)) != 0 && extXdetector(*(file_sys.super)) > 2){
        print_dash(5);
        printf("Filesystem is: EXT%d", extXdetector(*(file_sys.super)));
        print_dash(5); 
        puts("\n");
    } else {
        fprintf(stderr, "Can not a define Ext2 filesystem, exit...\n");
        exit(1);
...
void print_deleted(struct struct_ext2_filsys filsys){ 
    int size = 0;
    struct inode_d_time * read_inos = all_deleted_inodes(filsys, &size); 
    struct ext2_inode inode; 
    if(size > 0){
        for(int i = 0; i < size; i++){
            ext2fs_read_inode(&filsys, read_inos[i].inode, &inode);
